...
{
  // draw icon
  GUI_DrawBuff(slot_xs[slot], slot_ys[slot], 20, 20, MODE_GREY, 0,0, icon);
  // draw box name
...
void UI_Main(bool init)
{
  ...
  if(init)
  {
    ...
    // draw icon
    GUI_DrawBuff(2,24,34,12,MODE_GREY,-1,-1,img_bandwidth34x12);
  ...
  if(vfilter != sTuner.Config.nBandFilter[sTuner.Radio.nRFMode] || vfiltsel != bFilterSel || init)
  {
    ...
    if(sTuner.Radio.nRFMode == RFMODE_FM)
      GUI_DrawBuff(2,24+14,34,8,MODE_MONO,COLOR_BLACK,vfiltsel? COLOR_LIGHT:COLOR_WHITE,img_filter_fm+34*vfilter);
    else
      GUI_DrawBuff(2,24+14,34,8,MODE_MONO,COLOR_BLACK,vfiltsel? COLOR_LIGHT:COLOR_WHITE,img_filter_am+34*vfilter);
  }
...
void UI_Menu(int8_t index, bool init)
{
  ...
  if(index > 0)
  {
    ...
    // ARROR UP
    GUI_DrawBuff(25, 3, 24,12,MODE_GREY,0,0,img_arrowUp12x);
  }
  if(index < MENU_MAIN_INDEX-1)
  {
    ...
    // ARROR DOWN
    GUI_DrawBuff(25, 23+24+6+20+6, 24,12,MODE_GREY,0,0,img_arrowDown12x);
  }
...
void UI_Display(int8_t index, bool init)
{
  ...
  if(aa)
  {
    ...
    {
      UI_DrawCheckBox(0,img_commonSet20x, "Low EMI Mode", true, sDisplay.emiFree);
      UI_DrawValueBox(1,img_commonSet20x,"Light Grey", true, sDisplay.greyLevel[0], "");
      UI_DrawValueBox(2,img_commonSet20x,"Dark Grey", true, sDisplay.greyLevel[1], "");
      GUI_Rectangle(32,72+2,160,20,1,COLOR_BLACK);
      GUI_FillBuff(33,72+3,39,19,COLOR_WHITE);
      GUI_FillBuff(72,72+3,40,19,COLOR_LIGHT);
      GUI_FillBuff(112,72+3,40,19,COLOR_DARK);
      GUI_FillBuff(152,72+3,40,19,COLOR_BLACK);
    }
  }
  ...
  switch(index)
  {
    ...
    case 4 :UI_DrawCheckBox(0,img_commonSet20x, "Low EMI Mode", true, sDisplay.emiFree);
    case 5 :UI_DrawValueBox(1,img_commonSet20x,"Light Grey", true, sDisplay.greyLevel[0], "");break;
    case 6 :UI_DrawValueBox(2,img_commonSet20x,"Dark Grey", true, sDisplay.greyLevel[1], "");break;
    default : break;
  }
  ...
  if(aa) // init or change page
  {
    ...
    switch(index)
    {
      case 0:{ // main vol / audio gain & balance
        ...
        GUI_DrawBuff(8,24+2,47,20,MODE_GREY,0,0,img_volGain47x20);
        GUI_Text(8+47+8,24+2,256,24+2+20,"Vol Gain:",&Font20, COLOR_BLACK, COLOR_WHITE);
        GUI_DrawBuff(8,48+2,47,20,MODE_GREY,0,0,img_balLeft47x20);
        GUI_Text(8+47+8,48+2,256,48+2+20,"Bal Left:",&Font20, COLOR_BLACK, COLOR_WHITE);
        GUI_DrawBuff(8,72+2,47,20,MODE_GREY,0,0,img_balRight47x20);
        GUI_Text(8+47+8,72+2,256,72+2+20,"Bal Right:",&Font20, COLOR_BLACK, COLOR_WHITE);
      };break;
      
      case 1:{ // dc block
        GUI_DrawBuff(8,24+2,44,44,MODE_GREY,0,0,img_filtHP44x44);
...
void UI_About(int8_t index, bool init)
{
  ...
  {
    GUI_ClearBuff(COLOR_WHITE);
    GUI_DrawBuff(187,28,41,41,MODE_GREY,COLOR_BLACK,COLOR_WHITE,img_qrcode);
