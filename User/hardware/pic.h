/**
  ************************************* Copyright ******************************   
  *                 (C) Copyright 2023,NyaKoishi,China,GCU.
  *                            All Rights Reserved
  *
  * @file      : pic.h
  * @author    : NyaKoishi
  * @version   : V1.0.0
  * @date      : 2023-05-31
  * @brief     : Header for pic.h file
  * @attention : 
  * 
  ******************************************************************************
 */



#ifndef __PIC_H_
#define __PIC_H_

#include <stdint.h>




extern const unsigned char img_antenna[];
extern const unsigned char img_bat[];
extern const unsigned char img_batLow[];
extern const unsigned char img_channel[];
extern const unsigned char img_dbu[];
extern const unsigned char img_spkMute[];
extern const unsigned char img_spk[];
extern const unsigned char img_volt[];
extern const unsigned char img_percent[];

extern const unsigned char img_num16x[];
extern const unsigned char img_dot16x[];
extern const unsigned char img_num48x[];
extern const unsigned char img_dot48x[];
extern const unsigned char img_unit[];

extern const unsigned char img_stereo30x24[];

extern const unsigned char img_bandwidth34x12[];
extern const unsigned char img_filter_am[];
extern const unsigned char img_filter_fm[];

extern const unsigned char img_bright20x[];
extern const unsigned char img_commonSet20x[];
extern const unsigned char img_contrast20x[];
extern const unsigned char img_invDisp20x[];
extern const unsigned char img_lightTime20x[];

extern const unsigned char img_arrowUp12x[];
extern const unsigned char img_arrowDown12x[];
extern const unsigned char img_arrowLeft11x16[];
extern const unsigned char img_arrowRight11x16[];
extern const unsigned char img_mDisplay50x[];
extern const unsigned char img_mAbout50x[];
extern const unsigned char img_mUntitled50x[];

extern const unsigned char img_volGain47x20[];
extern const unsigned char img_balLeft47x20[];
extern const unsigned char img_balRight47x20[];
extern const unsigned char img_filtHP44x44[];
extern const unsigned char img_ultraBass44x44[];

extern const unsigned char img_qrcode[];
#endif




