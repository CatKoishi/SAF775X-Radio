...
// 6.56ms / update
// 每次同步LCD显存需要6.56ms，最大同步速率(150|30)FPS (PSC = 8, F = 7.5MHz | PSC = 32, F = 1.875MHz)
...
void lcd_set_window(uint8_t xs, uint8_t pgs, uint8_t xe, uint8_t pge)
{
  lcd_write_cmd(0x75, 2, pgs+16, pge+16);
  
  lcd_write_cmd(0x15, 2, xs, xe);
}
...
{
  uint32_t ticks;
  if(sDisplay.emiFree == false)  // regular full screen update
  {
    // wait for dma transmit complete
    ticks = sDevice.sTime.timestamp;
    while(dma_trans_flag)
    {
      if(sDevice.sTime.timestamp-ticks > 10)
        return;
    }
  ...
  else  // part update
  {
    ...
    lcd_set_window(xmin, pgs, xmax, pge);
    lcd_write_cmd(0x5c, 0);
    CS_PIN_LOW;
    DC_PIN_HIGH;
    dma_channel_disable(DMA0, DMA_CH4);
    dma_transfer_number_config(DMA0, DMA_CH4, k);
    dma_memory_address_config(DMA0, DMA_CH4, (uint32_t)LCD_Buff_CPY);
    dma_channel_enable(DMA0, DMA_CH4);
    
    xmin = LCD_DRV_MAX_X-1;
    ymin = LCD_DRV_MAX_Y-1;
    xmax = 0;
    ymax = 0;
  }
...
void lcd_dma_callback(void)
{
  if(dma_trans_flag == 1)
  {
    while(SET == spi_i2s_flag_get(SPI1, SPI_FLAG_TRANS));  // wait for final byte transmit complete
    CS_PIN_HIGH;
    dma_trans_flag = 0;
  }
}
...
void GUI_WriteXY(uint8_t xs, uint8_t ys, uint16_t xe, uint8_t ye)
{
  if(xs < xmin)
    xmin = xs;
  if(ys < ymin)
...
void GUI_FillBuff_Origin(uint16_t xs, uint8_t ys, uint16_t x ,uint8_t y, uint8_t data)
{
  ...
  GUI_WriteXY(xs,ys,xs+x-1,ys+y-1);
}
...
void GUI_Char(uint16_t xs, uint8_t ys, char ascii, sFONT* font, uint8_t color, uint8_t bcolor)
{
  ...
  GUI_DrawBuff(xs, ys, font->Width, font->Height, MODE_MONO, color, bcolor, ptr);
...
void DispFill(uint8_t page ,uint8_t column ,uint16_t x ,uint8_t y ,uint8_t color)
{
  ...
  lcd_set_window(column, page, column+x-1, page+height-1);
...
void LCD_StructInit(bool initPara)
{
  ...
  if(initPara)
  {
    ...
    sDisplay.brightness = 20;
    sDisplay.contrast = 265;
    sDisplay.greyLevel[0] = 16;
    sDisplay.greyLevel[1] = 20;
    sDisplay.invDisp = false;
    sDisplay.emiFree = false;
  }
}
